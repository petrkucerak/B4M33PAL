...
void split2groups(vector<Atom> &G, vector<vector<int>> &groups,
                  vector<vector<int>> &a_neighbour_groups, int count_atoms)

{
   ...
   int normal_id = 0;
   // int fast_id = count_atoms;
   std::vector<bool> used(count_atoms, false);
   for (int atomIndex = 0; atomIndex < count_atoms; ++atomIndex) {
                      ...
                      int count_atoms)
{
   ...
   for (int a_source = 0; a_source < count_atoms; ++a_source) {
...
bool permutation(vector<Atom> &A, vector<Atom> &B,
                 vector<vector<int>> &a_groups, vector<vector<int>> &b_groups,
                 int id, vector<int> &A2B, int count_atoms)
{
   ...
   if ((long unsigned int)(id + 1) >= a_groups.size()) {
      do {
         ...
         if (validate_mapping(A, B, A2B, count_atoms))
            return true;
      ...
      do {
         ...
         if (permutation(A, B, a_groups, b_groups, id + 1, A2B, count_atoms))
            return true;
      } while (next_permutation(loc_A_group.begin(), loc_A_group.end()));
   }
   return false;
...
int main(int argc, char const *argv[])
{
   ...
   vector<long unsigned int> molecule_tokens(count_mulecules, 0);
